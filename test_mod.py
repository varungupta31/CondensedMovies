...
def main(config):
    ...
    with torch.no_grad():
        for batch_idx, (minibatch, id) in tqdm(enumerate(data_loader), desc="First Iteration To Prepare the Content Embedding and MoE weights."):
    ...
    missing = torch.cat(missing, dim=0)
    ...
    with torch.no_grad():
        ...
        for batch_idx, (minibatch, id) in tqdm(enumerate(data_loader), desc='Second Iteration to Compute the Similarity Matrix'):
            for expert, subdict in minibatch.items():
                for key, val in subdict.items():
                    minibatch[expert][key] = val.to(device)
            ...
            sims, _, _  = model.forward_retrieve_one(minibatch, content_embeddings, missing)
