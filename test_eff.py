...
def main(config):
    ...
    #Second Dataloader for stochastic processing of the samples, for mem-efficient retrieval.
    config._config['data_loader']['args']['split'] = 'test'
    config._config['data_loader']['args']['batch_size'] = 1
    config._config['data_loader']['args']['shuffle'] = False
    data_loader_stoch = config.initialize('data_loader', module_data)
    ...
    #First loop to create the content embeddings.
    with torch.no_grad():
        for batch_idx, (minibatch, id) in enumerate(data_loader):
            ...
            res, miss = model.forward_video(minibatch)
    ...
    sims_matrix = []
    #Second loop for iterative completion of the similarity matrix.
    for batch_idx, (minibatch, id) in tqdm(enumerate(data_loader_stoch), desc='Second Iteration to Compute the Similarity Matrix'):
            for expert, subdict in minibatch.items():
                for key, val in subdict.items():
                    minibatch[expert][key] = val.to(device)
            sims, label, weights  = model.forward_retrieve_one(minibatch, content_embeddings, missing)
